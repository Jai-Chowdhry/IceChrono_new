...
def basinhopping(func, x0, niter=100, T=1.0, stepsize=0.5,
    ...
    if take_step is not None:
        ...
        if hasattr(take_step, "stepsize"):
            take_step_init = take_step(stepsize=stepsize)  # Fix proposed on 6/2/2017
            take_step_wrapped = AdaptiveStepsize(take_step_init, interval=interval,
                                                 verbose=disp)
        else:
            take_step_init = take_step()  # Fix proposed on 6/2/2017
            take_step_wrapped = take_step_init
    else:
        # use default
